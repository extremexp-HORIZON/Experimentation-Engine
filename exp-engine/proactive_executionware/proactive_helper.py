...
def save_artifacts(variables, *data):
    for (key, value) in data:
        save_artifact(variables, key, value)


def load_artifacts(variables, *keys):
    return [load_artifact(variables, key) for key in keys]


def save_artifact(variables, key, value):
    value_size = sys.getsizeof(value)
    print(f"Saving output data of size {value_size} with key {key}")
    if value_size > 100:
        print("Saving as large artifact")
        save_large_artifact(variables, key, value)
    else:
        print("Saving using the 'variables' object (internal database)")
        variables.put(key, value)


def load_artifact(variables, key):
    print(f"Loading input data with key {key}")
    if key in variables:
        print("Loading using the 'variables' object (internal database)")
        return variables.get(key)
    else:
        print("Loading as large artifact")
        return load_large_artifact(variables, key)


def save_large_artifact(variables, key, value):
...
def load_large_artifact(variables, key):
    job_id = variables.get("PA_JOB_ID")
